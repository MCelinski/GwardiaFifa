alter table public.tournament_predictions
  add column if not exists runner_up_team_id uuid references public.teams(id),
  add column if not exists third_place_team_id uuid references public.teams(id);

update public.tournament_predictions
set runner_up_team_id = coalesce(
  runner_up_team_id,
  case
    when finalist_a_team_id is distinct from champion_team_id then finalist_a_team_id
    else finalist_b_team_id
  end
)
where runner_up_team_id is null;

alter table public.tournament_predictions
  drop constraint if exists tournament_predictions_check,
  drop constraint if exists tournament_predictions_check1,
  add constraint tournament_predictions_podium_distinct_check check (
    champion_team_id is null
    or runner_up_team_id is null
    or third_place_team_id is null
    or (
...
begin
  ...
  where league_id = pred.league_id
    and stage = 'knockout'
    and lower(coalesce(round, '')) like '%third%'
    and status = 'finished'
  order by starts_at desc
  limit 1;
